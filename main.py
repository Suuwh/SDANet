...
def main():
    ...
    loader_args['d_label'], loader_args['l_label'], loader_args['d_spk2utt'] = make_d_label_spk2uttr(
        loader_args['dev_lines'])
...
def main_worker(gpu, ngpus_per_node, args, loader_args):
    ...
    if args.eval:
        ...
        for i, evalset_gen in enumerate(l_evalset_gen):
            ...
            # l_all_ID.append(l_ID)
            # l_embd.append(l_embedding)
        ...
        if gpu == 0:
            ...
            l_eer, l_min_dcf = sv_l([d_embeddings_all, d_embeddings_1, d_embeddings_2, d_embeddings_5],
                                    dic_eval_trial['voxceleb1_eval'], args)
    ...
    # Train the model#
        ...
        # args.metric_l = softmax
        if args.use_clf_l:
            if args.clf_l == 'softmax':
                criterion['clf_l'] = Softmax(nOut=args.model['code_dim'], nClasses=args.model['nb_spk']).cuda(gpu)
            elif args.clf_l == 'am':
                criterion['clf_l'] = AMSoftmax(nOut=args.model['code_dim'], nClasses=args.model['nb_spk'],
                                               margin=args.loss_margin, scale=args.loss_scale, gpu=gpu).cuda(gpu)
            elif args.clf_l == 'aam':
                criterion['clf_l'] = AAMSoftmax(nOut=args.model['code_dim'], nClasses=args.model['nb_spk'],
                                                margin=args.loss_margin, scale=args.loss_scale, gpu=gpu).cuda(gpu)
            elif args.clf_l == 'circle':
                # #(512,9)
                # feats = torch.rand(args.model['code_dim'], args.model['nb_spk'])
                # #(512,)
                # labels = torch.randint(high=10, dtype=torch.long, size=(args.model['code_dim'],))
                # circleloss = CircleLoss(margin=args.loss_margin, scale=args.loss_scale,similarity='cos').cuda(gpu)
                # criterion['clf_l'] = circleloss(feats, labels)
                criterion['clf_l'] = CircleLoss(nOut=args.model['code_dim'], nClasses=args.model['nb_spk'],
                                                margin=args.loss_margin, scale=args.loss_scale, gpu=gpu).cuda(gpu)
            elif args.clf_l == 'dlb':
                criterion['clf_l'] = DLBLoss(nOut=args.model['code_dim'], nClasses=args.model['nb_spk']).cuda(gpu)
            else:
                criterion['clf_l'] = None

        optimizer, lr_scheduler = get_optimizer(args, model, criterion)


        [model, criterion['clf_l']], optimizer = amp.initialize([model, criterion['clf_l']], optimizer,
                                                                    opt_level=args.opt_level)
        criterion['clf_l'] = nn.parallel.DistributedDataParallel(criterion['clf_l'], device_ids=[gpu],
