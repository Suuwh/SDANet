...
def train_model(model, db_gen, optimizer, epoch, args, lr_scheduler, criterion, gpu, last_images ):
    model.train()
    ...
    with tqdm(total=len(db_gen), ncols=150) as pbar:
        for idx, (m_batch, m_label) in enumerate(db_gen):
            ...
            if last_images is not None:
                m_batch = torch.cat([m_batch, last_images], dim=0)
                # m_label = torch.cat([m_label,last_lable], dim=0)
                code = model(m_batch)
                loss_clf = criterion['clf_l'](code, m_label, m_batch, epoch, last=True)
            else:
                code = model(m_batch)
                loss_clf = criterion['clf_l'](code, m_label, m_batch, epoch)
            ...
            # Update last
            last_images = m_batch[:batch_size]
        ...
        with tqdm(total=len(db_gen), ncols=70) as pbar:
            for (m_batch, ID) in db_gen:
                ...
                if m_batch.size(-1) == 59049:
                    ...
                    l_embeddings.extend(code.cpu().numpy())
                l_ID.extend(ID)
